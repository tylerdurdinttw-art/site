...
function buildWhere(filters: BanFilters): Prisma.BanWhereInput {
...
export async function listBans(filters: BanFilters): Promise<BansResponse> {
  const [rows, total, active, servers] = await Promise.all([
    ...
    prisma.server.findMany({ select: { id: true, name: true }, orderBy: { name: 'asc' } }),
  ]);
...
export async function resolveBanAdmin(
  ...
  const team = await prisma.playerCheck.findFirst({
    where: { status: 'finished', outcome: 'team_ban', reason, finishedAt: { gte: since } },
    select: { id: true },
  });

  return team ? PANEL_ADMIN : null;
}
